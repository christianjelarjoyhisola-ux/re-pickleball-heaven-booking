...
/**
 * Loads local account setup secrets from .env.local without requiring dotenv.
 *
 * @returns {Record<string, string>} Parsed key/value environment overrides.
 */
...
/**
 * Reads a required account setup value from .env.local or process.env.
 *
 * @param {Record<string, string>} env Local environment values.
 * @param {string} key Required environment variable name.
...
function requireEnv(env, key) {
  ...
  if (!value) throw new Error(`${key} is required. Copy .env.example to .env.local and set the R&E value.`);
  return value;
}
...
const ACCOUNTS = [
  { email: env.OWNER_EMAIL || 'owner@repickleballheaven.local', password: env.OWNER_PASSWORD || 'CHANGE_THIS_PASSWORD!', username: 'sysowner', full_name: 'System Owner', role: 'owner' },
  { email: env.COURT_OWNER_EMAIL || 'courtowner@repickleballheaven.local', password: env.COURT_OWNER_PASSWORD || 'CHANGE_THIS_PASSWORD!', username: 'courtowner', full_name: 'Court Owner', role: 'court_owner' },
  { email: env.STAFF_EMAIL || 'staff@repickleballheaven.local', password: env.STAFF_PASSWORD || 'CHANGE_THIS_PASSWORD!', username: 'courtstaff', full_name: 'Court Staff', role: 'staff' },
];

for (const account of ACCOUNTS) {
  if (account.password === 'CHANGE_THIS_PASSWORD!') {
    throw new Error(`Set a real temporary password for ${account.email} in .env.local before creating accounts.`);
  }
}
