...
### Added
- **Cloudflare Pages project** - created `re-pickleball-heaven` under the Cloudflare account and deployed the static public site.
...
### Added
- **Google AdSense** — site `repickleballheaven.local` connected, verified, and approved ("Ready"). AdSense code snippet added to the `<head>` of `index.html`, `login.html`, and `admin.html` (publisher `ca-pub-YOUR_ADSENSE_ID`)
