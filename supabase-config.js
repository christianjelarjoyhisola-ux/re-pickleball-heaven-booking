...
// =============================================
const RPE_CONFIG = window.RPE_CONFIG || {};
const SUPABASE_URL = String(RPE_CONFIG.SUPABASE_URL || '').trim();
const SUPABASE_ANON_KEY = String(RPE_CONFIG.SUPABASE_ANON_KEY || '').trim();

// Initialize Supabase client only when this R&E deployment has its own credentials.
const _sb = SUPABASE_URL && SUPABASE_ANON_KEY ? supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY) : null;
...
if (!_sb && !window.PB_USE_LOCAL_DATA) {
  console.warn('R&E Supabase credentials are missing. Fill in env.js, or use ?localData=1 on localhost for demo mode.');
}
...
(function installLocalDataMode() {
  ...
  const defaultAccounts = () => ([{
    ...
    email: 'owner@repickleballheaven.local',
    createdAt: nowIso(),
  }]);
